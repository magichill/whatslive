...
pid=`ps -eo pid,args | grep whatslive-api | grep $1 | grep java | grep -v grep | awk '{print $1}'`
...
java -Dio.netty.leakDetectionLevel=advanced -Dmodule=$1 $JAVA_OPTS -cp $PHOME/conf:$PHOME/lib/* com.letv.whatslive.server.Main $1 > /dev/null 2>&1 &
