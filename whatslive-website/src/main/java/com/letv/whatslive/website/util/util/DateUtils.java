...
public class DateUtils {
     ...
     * @return
     */
    public static String getTimeLength(Long startTime, Long endTime) {

        Long timeInterval = endTime - startTime;
        String hours = ObjectUtils.toString(timeInterval / ONE_HOUR_MILLISECOND);
        String minutes = ObjectUtils.toString((timeInterval % ONE_HOUR_MILLISECOND) / ONE_MINUTE_MILLISECOND);
        String seconds = ObjectUtils.toString(((timeInterval % ONE_HOUR_MILLISECOND) % ONE_MINUTE_MILLISECOND) / ONE_SECOND_MILLISECOND);

        StringBuffer timeLength = new StringBuffer();
        if (hours.length() == 1) {

            timeLength.append("0");
        }
        timeLength.append(hours);
        timeLength.append(":");
        if (minutes.length() == 1) {

            timeLength.append("0");
        }
        timeLength.append(minutes);
        timeLength.append(":");
        if (seconds.length() == 1) {

            timeLength.append("0");
        }
        timeLength.append(seconds);
