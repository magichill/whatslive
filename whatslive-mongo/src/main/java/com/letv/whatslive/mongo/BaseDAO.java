...
public abstract class BaseDAO {
    ...
    public DBObject likeFind(String keyword,String fieldName){
        BasicDBObject dbObject = new BasicDBObject();
        Pattern pattern = Pattern.compile("^.*" + keyword+ ".*$", Pattern.CASE_INSENSITIVE);
        dbObject.put(fieldName, pattern);
        return this.getDbCollection().findOne(dbObject);
    }

    public List<DBObject> likeFindAll(String keyword,String fieldName){
        BasicDBObject dbObject = new BasicDBObject();
        List result = Lists.newArrayList();
        DBCursor cur = null;
        Pattern pattern = Pattern.compile("^.*" + keyword+ ".*$", Pattern.CASE_INSENSITIVE);
        dbObject.put(fieldName, pattern);
        try {
            cur = this.getDbCollection().find(dbObject);
            while (cur.hasNext()) {
                result.add(cur.next());
            }
        }catch(Exception e){
